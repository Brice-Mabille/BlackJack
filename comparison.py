...
from Agent_Sarsa import train_sarsa_agent
...
trained_q_learning_agent, scores, final_hands = train_q_learning_agent(episodes=10000)
...
episodes = 10000
random_wins = test_agent(random_agent_turn, episodes)
simple_wins = test_agent(agent_turn, episodes)
q_learning_wins = test_agent(q_learning_agent, episodes)
sarsa_wins = test_agent(SARSA_agent, episodes)

# Tracer les courbes
plt.figure()
plt.plot(range(0, episodes+1), random_wins, label="Random Agent", color="red")
